...
export default function SignupPage() {
  ...
  }, [showSuccessModal, router, tenantAuthPath]);
    ...
    if (!inviteToken) {
      return (
        <div className="rounded-2xl border border-rose-200 bg-rose-50/60 p-6 text-sm text-rose-700 dark:border-rose-500/40 dark:bg-rose-500/10 dark:text-rose-200">
          A valid invitation link is required to create an account. Please check
          your email for the latest invite or contact your HR administrator.
        </div>
      );
    }
      ...
      return (
        <div className="rounded-2xl border border-rose-200 bg-rose-50/60 p-6 text-sm text-rose-700 dark:border-rose-500/40 dark:bg-rose-500/10 dark:text-rose-200">
          {inviteQuery.error?.message ??
            "We couldn't validate your invitation. Please request a new link."}
        </div>
      );
    }
  ...
  const heading = inviteQuery.data
    ? `Join ${inviteQuery.data.organizationName}`
    : "Accept your invitation";
  const supportingCopy = inviteQuery.data
    ? "Review the details provided by your HR team, upload a profile photo, and choose a password."
    : "Use the secure link in your email to confirm your details and set a password.";
  ...
  return (
    <div className="space-y-8">
      <div className="space-y-2">
        ...
        />
        <p className="text-sm text-text_secondary">{supportingCopy}</p>
      </div>
      {content}
      {showSuccessModal ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/70 px-4">
          <div className="w-full max-w-md space-y-4 rounded-3xl border border-white/20 bg-white p-8 text-center shadow-2xl dark:border-slate-700 dark:bg-slate-900">
            ...
            <p className="text-sm text-text_secondary">
              Your account is ready. You’ll be redirected to login so you can

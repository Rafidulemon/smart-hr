...
export default function EmployeeProfilePage() {
  ...
  if (!employeeId) {
    return (
      <EmptyState
        message="Pick an employee from the directory to view their profile."
        directoryHref={tenantPath("/hr-admin/employees")}
      />
    );
  }
  ...
  if (profileQuery.isError || !profileQuery.data?.profile) {
    return (
      <EmptyState
        message="We couldn’t load the employee profile. Try opening it from the directory again."
        directoryHref={tenantPath("/hr-admin/employees")}
      />
    );
  }
    ...
    <div className="space-y-8">
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap gap-3">
          <Link
            href={tenantPath(`/hr-admin/employees/edit/${encodeURIComponent(employee.id)}`)}

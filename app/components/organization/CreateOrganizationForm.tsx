...
type CreateOrganizationFormProps = {
  returnHref: string;
  returnLabel?: string;
  limitToFirstOrganization?: boolean;
};

export default function CreateOrganizationForm({
  returnHref,
  returnLabel = "Back to organization settings",
  limitToFirstOrganization = false,
  ...
  const organizationListQuery = trpc.hrOrganization.list.useQuery(undefined, {
    ...
    enabled: limitToFirstOrganization,
  });
  const creationLocked = limitToFirstOrganization
    ? (organizationListQuery.data?.organizations.length ?? 0) > 0
    : false;
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (
      ...
      setAlert({
        type: "error",
        message:
          "Sub-domain must be 3-63 characters, lowercase letters, numbers, or hyphens, and cannot start or end with a hyphen.",
      });
      return;
    }
    setInviteUrl(null);
    ...
    <div className="space-y-8">
      <div className="rounded-[32px] border border-white/70 bg-white/90 p-6 shadow-xl shadow-indigo-100 dark:border-slate-700/70 dark:bg-slate-900/80 dark:shadow-slate-950/60">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          ...
          <Button href={returnHref} className="rounded-2xl px-5 py-2 text-sm">
            {returnLabel}
          </Button>
        </div>
      </div>
      ...
      {limitToFirstOrganization && creationLocked ? (
      ...
      >
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">
            Used in tenant URLs like /org/acme. Only lowercase letters, numbers, and hyphens.
          </p>
        </div>
        <TextInput
